...
def get_IAT_features(path,hasher):
    ...
    func_features = []
    hashed_features = hasher.transform([func_features])
    ...
    objlist = dir(pe)
    if 'DIRECTORY_ENTRY_IMPORT' in objlist:
        for entry in pe.DIRCTORY_ENTRY_IMPORT:
            for func in entry.imports:
                func_features[str(func.name)]=1
    else:
        return hashed_features
    ...
    print("Extracted {0} function from {1".format(len(func_features), path))
    return hashed_features
